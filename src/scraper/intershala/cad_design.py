...
logger = logging.getLogger("intershala.cad_design")


async def scrape_internshala_cad_design():
    url = "https://internshala.com/internships/cad-design-internship/"
    ...
    for card in cards:
        try:
            ...
            job.update(
                {
                    ...
                    "embedding_text": (
                        ...
                        "cad design mechanical design engineering design autocad "
                        "solidworks fusion 360 drafting 3d modeling"
                    ).strip(),
                    "raw_payload": {},
                    "extra_metadata": {
                        "posted_text": posted,
            ...
            logger.error("Internshala CAD Design Parse Error: %s", exc)

    logger.info("Scraped %d CAD design internships from Internshala.", len(internships))
    return internships


if __name__ == "__main__":
    jobs = asyncio.run(scrape_internshala_cad_design())
    print(f"Scraped {len(jobs)} CAD design internships from Internshala.")
