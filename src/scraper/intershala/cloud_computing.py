...
logger = logging.getLogger("intershala.cloud_computing")


async def scrape_internshala_cloud_computing():
    ...
    for card in cards:
        try:
            ...
            job.update(
                {
                    ...
                    "embedding_text": (
                        ...
                        "cloud computing devops infrastructure aws azure gcp docker "
                        "kubernetes linux ci cd cloud deployment"
                    ).strip(),
                    "raw_payload": {},
                    "extra_metadata": {
                        "posted_text": posted,
            ...
            logger.error("Internshala Cloud Computing Parse Error: %s", exc)

    logger.info(
        "Scraped %d cloud computing internships from Internshala.",
        len(internships),
    )
    return internships
...
if __name__ == "__main__":
    jobs = asyncio.run(scrape_internshala_cloud_computing())
    print(f"Scraped {len(jobs)} cloud computing internships from Internshala.")
