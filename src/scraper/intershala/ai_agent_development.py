...
logger = logging.getLogger("intershala.ai_agent_development")


async def scrape_internshala_ai_agent_development():
    url = "https://internshala.com/internships/ai-agent-development-internship/"
    ...
    for card in cards:
        try:
            ...
            stipend_text = (
                stipend_tag.get_text(strip=True).replace("â‚¹", "Rs.")
                if stipend_tag
                else None
            )
            ...
            job.update(
                {
                    ...
                    "embedding_text": (
                        ...
                        "AI agent development LLM LangChain LangGraph RAG automation"
                    ).strip(),
                    "raw_payload": {},
                    "extra_metadata": {
                        "posted_text": posted,
            ...
            logger.error("Internshala AI Agent Parse Error: %s", exc)

    logger.info(
        "Scraped %d AI agent development internships from Internshala.",
        len(internships),
    )
    return internships
...
if __name__ == "__main__":
    jobs = asyncio.run(scrape_internshala_ai_agent_development())
    print(f"Scraped {len(jobs)} AI agent development internships from Internshala.")
