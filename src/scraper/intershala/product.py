...
logger = logging.getLogger("intershala.product")


async def scrape_internshala_product():
    url = "https://internshala.com/internships/product-internship/"
    ...
    for card in cards:
        try:
            ...
            logger.error("Internshala Product Parse Error: %s", exc)

    logger.info("Scraped %d product internships from Internshala.", len(internships))
    return internships


if __name__ == "__main__":
    jobs = asyncio.run(scrape_internshala_product())
    print(f"Scraped {len(jobs)} product internships from Internshala.")
