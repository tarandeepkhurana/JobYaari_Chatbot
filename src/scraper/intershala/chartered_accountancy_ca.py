...
logger = logging.getLogger("intershala.chartered_accountancy_ca")


async def scrape_internshala_chartered_accountancy_ca():
    url = "https://internshala.com/internships/chartered-accountancy-ca-internship/"
    ...
    for card in cards:
        try:
            ...
            job.update(
                {
                    ...
                    "embedding_text": (
                        ...
                        "chartered accountancy ca accounting audit taxation gst "
                        "bookkeeping finance compliance tally"
                    ).strip(),
                    "raw_payload": {},
                    "extra_metadata": {
                        "posted_text": posted,
            ...
            logger.error("Internshala Chartered Accountancy CA Parse Error: %s", exc)

    logger.info(
        "Scraped %d chartered accountancy CA internships from Internshala.",
        len(internships),
    )
    return internships
...
if __name__ == "__main__":
    jobs = asyncio.run(scrape_internshala_chartered_accountancy_ca())
    print(f"Scraped {len(jobs)} chartered accountancy CA internships from Internshala.")
