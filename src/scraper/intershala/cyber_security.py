...
logger = logging.getLogger("intershala.cyber_security")


async def scrape_internshala_cyber_security():
    ...
    for card in cards:
        try:
            ...
            logger.error("Internshala Cyber Security Parse Error: %s", exc)

    logger.info(
        "Scraped %d cyber security internships from Internshala.",
        len(internships),
    )
    return internships
...
if __name__ == "__main__":
    jobs = asyncio.run(scrape_internshala_cyber_security())
    print(f"Scraped {len(jobs)} cyber security internships from Internshala.")
