...
logger = logging.getLogger("intershala.front_end_development")


async def scrape_internshala_front_end_development():
    ...
    for card in cards:
        try:
            ...
            logger.error("Internshala Front End Development Parse Error: %s", exc)

    logger.info(
        "Scraped %d front end development internships from Internshala.",
        len(internships),
    )
    return internships
...
if __name__ == "__main__":
    jobs = asyncio.run(scrape_internshala_front_end_development())
    print(f"Scraped {len(jobs)} front end development internships from Internshala.")
