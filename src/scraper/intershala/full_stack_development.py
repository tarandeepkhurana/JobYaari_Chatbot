...
logger = logging.getLogger("intershala.full_stack_development")


async def scrape_internshala_full_stack_development():
    ...
    for card in cards:
        try:
            ...
            logger.error("Internshala Full Stack Development Parse Error: %s", exc)

    logger.info(
        "Scraped %d full stack development internships from Internshala.",
        len(internships),
    )
    return internships
...
if __name__ == "__main__":
    jobs = asyncio.run(scrape_internshala_full_stack_development())
    print(f"Scraped {len(jobs)} full stack development internships from Internshala.")
