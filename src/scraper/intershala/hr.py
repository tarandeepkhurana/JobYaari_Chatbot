...
logger = logging.getLogger("intershala.hr")


async def scrape_internshala_hr():
    url = "https://internshala.com/internships/hr-internship/"
    ...
    for card in cards:
        try:
            ...
            logger.error("Internshala HR Parse Error: %s", exc)

    logger.info("Scraped %d HR internships from Internshala.", len(internships))
    return internships


if __name__ == "__main__":
    jobs = asyncio.run(scrape_internshala_hr())
    print(f"Scraped {len(jobs)} HR internships from Internshala.")
