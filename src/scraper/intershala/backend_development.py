...
logger = logging.getLogger("intershala.backend_development")


async def scrape_internshala_backend_development():
    ...
    for card in cards:
        try:
            ...
            job.update(
                {
                    ...
                    "embedding_text": (
                        ...
                        "backend development api development server side python "
                        "fastapi django flask node.js database postgresql"
                    ).strip(),
                    "raw_payload": {},
                    "extra_metadata": {
                        "posted_text": posted,
            ...
            logger.error("Internshala Backend Development Parse Error: %s", exc)

    logger.info(
        "Scraped %d backend development internships from Internshala.",
        len(internships),
    )
    return internships
...
if __name__ == "__main__":
    jobs = asyncio.run(scrape_internshala_backend_development())
    print(f"Scraped {len(jobs)} backend development internships from Internshala.")
