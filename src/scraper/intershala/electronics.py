...
logger = logging.getLogger("intershala.electronics")


async def scrape_internshala_electronics():
    ...
    for card in cards:
        try:
            ...
            job.update(
                {
                    ...
                    "embedding_text": (
                        ...
                        "electronics embedded systems hardware engineering pcb "
                        "microcontroller arduino raspberry pi circuit design iot"
                    ).strip(),
                    "raw_payload": {},
                    "extra_metadata": {
                        "posted_text": posted,
            ...
            logger.error("Internshala Electronics Parse Error: %s", exc)

    logger.info("Scraped %d electronics internships from Internshala.", len(internships))
    return internships


if __name__ == "__main__":
    jobs = asyncio.run(scrape_internshala_electronics())
    print(f"Scraped {len(jobs)} electronics internships from Internshala.")
