...
logger = logging.getLogger("intershala.teaching")


async def scrape_internshala_teaching():
    url = "https://internshala.com/internships/teaching-internship/"
    ...
    for card in cards:
        try:
            ...
            logger.error("Internshala Teaching Parse Error: %s", exc)

    logger.info("Scraped %d teaching internships from Internshala.", len(internships))
    return internships


if __name__ == "__main__":
    jobs = asyncio.run(scrape_internshala_teaching())
    print(f"Scraped {len(jobs)} teaching internships from Internshala.")
