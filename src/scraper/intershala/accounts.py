...
logger = logging.getLogger("intershala.accounts")


async def scrape_internshala_accounts():
    url = "https://internshala.com/internships/accounts-internship/"
    ...
    for card in cards:
        try:
            ...
            job.update(
                {
                    ...
                    "skills": skills,
                    "categories": ["accounts", "accounting", "finance"],
                    ...
                    "embedding_text": (
                        ...
                        "accounts accounting finance bookkeeping taxation audit"
                    ).strip(),
                    "raw_payload": {},
                    "extra_metadata": {
                        "posted_text": posted,
            ...
            logger.error("Internshala Accounts Parse Error: %s", exc)

    logger.info("Scraped %d accounts internships from Internshala.", len(internships))
    return internships


if __name__ == "__main__":
    jobs = asyncio.run(scrape_internshala_accounts())
    print(f"Scraped {len(jobs)} accounts internships from Internshala.")
